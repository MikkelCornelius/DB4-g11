...
async def measure_OD(measurement_count, measurement_interval, pump_duration):
    ...
    if time.ticks_ms() - last_feeding_time > (T*60000):
        ...
        if dV>0 and dV<200000:
            ...
            file_od.flush()
            if manual_control:
                pump_volume_manual(PUMP_B, pump_duration)
            else:
                await pump_volume_auto(PUMP_B, dV)
        else:
...
async def pump_volume_auto(pump, vol):
...
async def pump_volume_manual(pump, vol):
    # pump speed is 18.5 ml/second at max power
    print("Feeding")
    pump.duty_u16(65535)
    await uasyncio.sleep(vol)
    print("Stopping feedstock")
    pump.duty_u16(0)
...
def cb(topic, msg):
    global pump_duration, wait, manual_control
    try:
        msg_value = msg.decode('utf-8')
        print('Message received:', msg_value)

        if topic == f'{ADAFRUIT_USERNAME}/feeds/out.manual'.encode():
            manual_control = msg_value==1
        elif topic == f'{ADAFRUIT_USERNAME}/feeds/out.pump-duration'.encode():
            pump_duration = int(msg_value)
        elif topic == f'{ADAFRUIT_USERNAME}/feeds/out.wait'.encode():
            wait = int(msg_value)
